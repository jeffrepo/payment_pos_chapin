...
class PoschapinController(http.Controller):
    ...
    @http.route('/shop/payment/testpos', type='http', auth='public', method='POST')
    def crear_test_ruta(self,**post):
        logging.warn('TEST RUTA')
    ...
    def poschapin_s2s_create_json_3ds(self, verify_validity=False, **kwargs):
        ...
        logging.warn('ACQUIRRER')
        logging.warn(acquirer)
        logging.warn('KWARGS')
        logging.warn(token)
    ...
    # @http.route('/shop/payment', type='http', auth='public', method='POST')
    def testa(self,data):
        logging.warn('HOLA')
        # logging.warn(extra_url)
    ...
    @http.route('/shop/exito',type="http",website=True,auth="user")
    def procesot_exito(self, **kw):
        logging.warn('extio')
        logging.warn(kw)
        # print (str(request.env.user.partner_id))
        return request.render("payment_pos_chapin.portal_exito",{})
